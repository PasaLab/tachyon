...
import tachyon.perf.benchmark.mixture.MixtureThread;
 ...
 */
public class LoopTaskContext extends SimpleTaskContext {
  @Override
  public void setFromThread(PerfThread[] threads) {
    ...
    for (PerfThread thread : threads) {
      if (!((LoopThread) thread).getSuccess()) {
        mSuccess = false;
      }
      basicThroughputs.add(((LoopThread) thread).getBasicWriteThroughput());
      readThroughputs.add(((LoopThread) thread).getReadThroughput());
      basicWriteTime.add(((LoopThread) thread).getBasicWriteTime());
      readTime.add(((LoopThread) thread).getReadTime());
    }
