...
public class ByteBufferKeyValuePartitionReaderPool {
  ...
  /** BlockWorker handler for accessing block info. */
   ...
   * @param blockWorker the blockworker is used to unlock a block
   */
  ...
  /**
   * Called by {@link #getReader(Long)}  when there is no matching reader in the pool
   * to the requested blockId.
   */
   ...
   * {@link FutureTask} to guarantee that the ReaderRecord can only be created once.
   *
   * @param blockId the requested blockId
   * @return the requested ByteBufferKeyValuePartitionReader
  ...
  private Future<ReaderRecord> put(final Long blockId,
    ...
    try {
      if (mReaderQueue.contains(blockId) && !isFullEldest(blockId)) {
        // Change the position of an existing key to the tail of the queue.
  ...
  /**
   * This must be called when the user finished using the ByteBufferKeyValuePartitionReader. When
   * the pool is full, it also needs to remove the eldest ReaderRecord.
  ...
  /**
   * Releases the resource of an ReaderRecord given an blockId. It closes
  ...
  /**
   * ByteBufferKeyValuePartitionReader pool's record.
   */
