...
public final class TieredBlockStore implements BlockStore {
   ...
   */
  public TieredBlockStore() throws IOException {
  ...
  private String printEvictorHitTrace(Map<EvictorType, Boolean> hit) {
    String trace = "";
    if (hit.containsKey(EvictorType.LRU)) {
      trace += EvictorType.LRU + "::" + hit.get(EvictorType.LRU) + "::";
    }
    if (hit.containsKey(EvictorType.LRFU)) {
      trace += EvictorType.LRFU + "::" + hit.get(EvictorType.LRFU) + "::";
    }
    if (hit.containsKey(EvictorType.LIRS)) {
      trace += EvictorType.LIRS + "::" + hit.get(EvictorType.LIRS) + "::";
    }
    if (hit.containsKey(EvictorType.ARC)) {
      trace += EvictorType.ARC + "::" + hit.get(EvictorType.ARC) + "::";
    }
    return trace;
  }
      ...
      if (moveResult.getSuccess()) {
        if (!mCandidate) {
          ...
          if (blockMeta != null) {
            ...
            LOG.info(Configuration.get(PropertyKey.TRACE_LOGGER_HEAD) + blockId + "::" + blockSize
                + "::move::");
            if (mEvictorAutoEnabled) {
  ...
  public void switchEvictor(EvictorType evictorType) {
    ...
    System.out.println("Switch from " + mEvictorType + " to " + evictorType);
  ...
  public boolean accessBlockReturnHit(long sessionId, long blockId)
    ...
    try (LockResource r = new LockResource(mMetadataReadLock)) {
      BlockMeta blockMeta = mMetaManager.getBlockMeta(blockId);
      hit = blockMeta.getParentDir().getParentTier().getTierOrdinal() == 0;
    }
  ...
  public void accessBlock(long sessionId, long blockId) throws BlockDoesNotExistException {
    ...
    }
    {
      ...
      if (blockMeta != null && blockMeta.getParentDir().getParentTier().getTierOrdinal() != 0) {
        try {
          if (mCandidate) {
    ...
    if (!mCandidate) {
      ...
      if (blockMeta != null) {
        long blockSize = blockMeta.getBlockSize();
        LOG.info(Configuration.get(PropertyKey.TRACE_LOGGER_HEAD) + blockId + "::" + blockSize
            + "::access::" + printEvictorHitTrace(hitListener));
        if (mEvictorAutoEnabled) {
  ...
  private void removeBlockInternal(long sessionId, long blockId, BlockStoreLocation location)
    ...
    try {
      String filePath;
      BlockMeta blockMeta;
      if (mCandidate && !hasBlockMeta(blockId)) {
        return;
      }
      try (LockResource r = new LockResource(mMetadataReadLock)) {
        if (mMetaManager.hasTempBlockMeta(blockId)) {
